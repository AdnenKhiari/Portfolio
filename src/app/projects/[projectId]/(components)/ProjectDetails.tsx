...
export default function ProjectDetails({
  ...
  return (
    <section className="project-instance">
      ...
      <div className="img-container">
        {img && (
          <Image
            ...
            src={img}
            fill
            alt={"Project i did or collaborated at"}
          />
        )}
      </div>
      <div className="project-header">
      <p className="p-light">
            {/* {data.projectDate && data.projectDate.toDateString()} */}
          </p>
        <div className="date-list-skills">
          ...
          {data.labels?.length > 0 && (
            <motion.div
              viewport={{ once: true }}
              variants={container_2}
              whileInView="animate"
              initial="initial"
              className="label-container"
            >
              {data.labels?.map((label: any, key: number) => (
                <motion.p variants={item} className="label" key={key}>
                  {label}
                </motion.p>
              ))}
            </motion.div>
      ...
      <article>{data.content && parse(data.content)}</article>
