...
const getIcons = async () => {
  ...
  const list = await Promise.all(
    listprefixes.prefixes.map((prefix) => {
      return listAll(ref(storage, prefix.fullPath));
    })
  ).then((arr: ListResult[]) => {
    return arr.map((i) => i.items).flat(1);
  });
...
export default function TechStackIcons() {
  ...
  useEffect(() => {
    ...
    if (window && window.width)
      setMaxIcons(
        Math.floor((window.width ?? 0) / (window.width < 700 ? 60 : 140))
      );
  }, [window, ref]);
  ...
  useEffect(() => {
    ...
    getIcons().then((data) => {
      ...
      for (let i = 0; i < data.length; i++) {
        arr.push([
          { url: data[i], name: "xD", categorie: "lel" },
          Math.random() * 1000000,
        ]);
      }
      setIcons(arr);
  ...
  return (
    ...
    >
      ...
      >
        {icons &&
          icons
            .filter((arr, index) => index < icons.length / 2)
